...
namespace Shim
{
    public class FuseSocket : MonoBehaviour
    {
        ...
        public void OnlightEnter(SelectEnterEventArgs args)
        {
            if (args.interactableObject.transform.gameObject.CompareTag("Fuse"))
            {
                // 퓨즈 아이템이 소켓에 삽입되었을 때
                fuseInserted = true;
        ...
        public void OnlightExit(SelectExitEventArgs args)
        {
            if (args.interactableObject.transform.gameObject.CompareTag("Fuse"))
            {
                // 퓨즈 아이템이 소켓에 삽입되었을 때
